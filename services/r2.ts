...
async function generatePresignedUrl(
    ...
    // Use us-east-1 for maximum compatibility with S3 clients (R2 supports this)
    ...
    const queryParams: Record<string, string> = {
        ...
        'X-Amz-Credential': `${R2_CONFIG.ACCESS_KEY_ID}/${credentialScope}`,
        'X-Amz-Date': amzDate,
    ...
    const signingKey = await getSignatureKey(R2_CONFIG.SECRET_ACCESS_KEY, dateStamp, region, service);
...
export async function uploadToR2(base64Data: string): Promise<string> {
    try {
        const bucketName = R2_CONFIG.BUCKET_NAME;
